...
export default  function TournamentDetailClient({competitionId}:Props) {
    ...
    async function fetchParticipations() {
      ...
      if (!Array.isArray(data)) {
        console.error("Unexpected participations data:", data);
      }
      ...
      const medalsByAthlete = await fetchMedals();
      console.log("Medals by athlete:", medalsByAthlete);
      if (medalsByAthlete == undefined) {
        console.error("Medals data is undefined");
        return;
      }
      data.forEach((p:any)=> {
        if (!grouped[p.athlete_id]) {
          ...
          grouped[p.athlete_id] = {
            ...
            medal: medalsByAthlete[p.athlete_id] || "none",
          };
    ...
    async function fetchMedals() {
      try {
        ...
        const data = await res.json();
        const medalsByAthlete: Record<number, "none" | "gold" | "silver" | "bronze"> = {};
        console.log("Fetched medals data:", data);
        data.forEach((m:any)=> {
          medalsByAthlete[m.athleteId] = m.medalType;
        })
        console.log("Processed medals by athlete:", medalsByAthlete);
                          ...
                          <td className="py-3">
                            <select
                              ...
                              onChange={e => {
                                ...
                                console.log(`Selected medal for athlete ${a.id}:`, value);
